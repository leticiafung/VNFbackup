...
class Fullbp(object):
	def __init__(self,fnode,fgraph,numofnode,mt):#传入node实例，graph实例，节点数
		self.node = []
		self.requires = []
	...
	def embedPrim(self,require1):#single require at a time,not backup embed ,require1[0] must be sorted!!!wrong
		self.node.sort(reverse = True)
...
if __name__ == '__main__':
		...
		requires = requestcreat(1,[0.95,0.99,0.999])
		# start = time.clock()
		#mybp = Fullbp()
		mybp = Fullbp(node,mygraph,nodes,2)
		print "success!"
		# end = time.clock()
		# end-start
		#多次
