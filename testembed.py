...
if __name__ == '__main__':
		...
		for algo in range(2):
			...
			#mybp = Fullbp()
			if algo  == 0:
				mybp = Fullbp(node,newgraph,nodes,2)#共享程度为2
			# elif algo == 1 :
			# 	mybp = GREP(node,mygraph,nodes,2)
			# elif algo == 2:
			# 	mybp = Fullbp(node,mygraph,nodes,3)
			else:
				mybp = JENA(node,newgraph,nodes,2)
			...
			for i in range(3):
				...
				T[i] = T[i]/ci[i]	
			if algo  == 0 :	
				f = open('mybpembed.txt','a')
			else:
				f = open('justeembed.txt','a')
			f.writelines('embed =' + str(algo) +'\n')#algo = 0,1原图，2,3新图
			f.writelines('\n'+ 'increase: ' + str(Y) +'\n'+'usedresource: ' + str(M) + '\n')
